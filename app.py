...
# ---------------- DEBUG PANEL ----------------
with st.expander("🧪 DEBUG INFO (Open this first)"):
    ...
    if has_old:
        st.error("❌ OLD SDK detected (google-generativeai). This forces v1beta and causes model 404.")
...
with st.expander("📜 DEBUG: List Available Models"):
    try:
        ...
        count = 0
        for m in models:
            st.write("MODEL:", m.name)
            count += 1
            if count >= 20:
                st.warning("Showing only first 20 models...")
                break
        if count == 0:
            st.warning("No models returned. This usually means API access/quota issue.")
    except Exception as e:
